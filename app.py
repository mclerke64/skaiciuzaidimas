...
def get_middle_players(players):
    ...
    if len(players) < 3:
        return []
    # Get unique guesses and sort them
    ...
    if len(unique_guesses) % 2 == 0:
        return []
    # Odd number of unique guesses: middle number is at len(unique_guesses) // 2
    middle_guess = unique_guesses[len(unique_guesses) // 2]
    print(f"Middle guess: {middle_guess}")
    # Return all players who guessed the middle number
...
def reset():
    ...
    session['game_id'] = game_id
    session['submitted'] = False
    socketio.emit('redirect_to_index', {}, namespace='/')
...
def countdown():
    ...
    while countdown_active and countdown_start_time is not None:
        ...
        if remaining_time <= 0:
            ...
            print(f"Countdown ended, winners: {winners}")
